...
def telecomments_messages_menu():
    ...
    # Check if the bot id matches with @TeleCommentsBot
    if data.get("id") != 547079958:
    ...
    for message in data.get("messages", []):
        if message.get("from_id") == "user547079958" and "inline_bot_buttons" in message:
            for button_row in message["inline_bot_buttons"]:
                for button in button_row:
                    if button.get("text") == "✍🏻 پاسخ":
    ...
    if messages:
        ...
        chat_id = base64decoder(button_data[selected_message], 1).split('a')[1].split('_')[0]
        message_id = base64decoder(button_data[selected_message], 1).split('_')[1]
...
def telecomments():
    print("Tutorial: ")
    print("  1. Export chat history of @TeleCommentsBot")
